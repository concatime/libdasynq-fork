...
#include "dasynq/flags.hpp"
#include "dasynq/interrupt.hpp"
#include "dasynq/stableheap.hpp"
#include "dasynq/util.hpp"

// Dasynq uses a "mix-in" pattern to produce an event loop implementation
// incorporating selectable implementations of various components (main backend,
// timers, child process watch mechanism etc). In C++ this can be achieved by a
// template for some component which extends its own type parameter:
//
//     template <typename Base> class X : public B { .... }
//
// (Note that in a sense this is actually the opposite of the so-called
// "Curiously Recurring Template" pattern, which can be used to achieve a
// similar goal). We can chain several such components together to "mix in" the
// functionality of each into the final class, eg:
//
//     template <typename T> using loop_t =
//         epoll_loop<interrupt_channel<timer_fd_events<child_proc_events<T>>>>;
//
// (which defines an alias template "loop_t", whose implementation will use the
// epoll backend, a standard interrupt channel implementation, a timerfd-based
// timer implementation, and the standard child process watch implementation).
// We sometimes need the base class to be able to call derived-class members: to
// do this we pass a reference to the derived instance into a template member
// function in the base, for example the "init" function:
//
...
//     }
//
// The 'loop_t' defined above is a template for a usable backend mechanism for
// the event_loop template class. At the base all this is the event_dispatch
// class, defined below, which receives event notifications and inserts them
// into a queue for processing. The event_loop class, also below, wraps this
// (via composition) in an interface which can be used to
// register/de-register/enable/disable event watchers, and which can process the
// queued events by calling the watcher callbacks. The event_loop class also
// provides some synchronisation to ensure thread-safety, and abstracts away
// some differences between backends.
//
// The differences are exposed as traits, partly via a separate traits class
// (loop_traits_t as defined below, which contains the "main" traits,
// particularly the sigdata_t, fd_r and fd_s types). Note that the
// event_dispatch class exposes the loop traits as traits_t, and these are then
// potentially augmented at each stage of the mechanism inheritance chain (i.e.
// the final traits are exposed as `loop_t<event_dispatch>::traits_t'.
//
// The trait members are:
//   sigdata_t  - a wrapper for the siginfo_t type or equivalent used to pass
//   signal parameters fd_r       - a file descriptor wrapper, if the backend is
//   able to retrieve the file descriptor when
//                it receives an fd event. Not all backends can do this.
//   fd_s       - a file descriptor storage wrapper. If the backend can retrieve
//   file descriptors, this
//                will be empty (and ideally zero-size), otherwise it stores a
//                file descriptor. With an fd_r and fd_s instance you can always
//                retrieve the file descriptor: `fdr.get_fd(fds)' will return
//                it.
//   has_bidi_fd_watch
//              - boolean indicating whether a single watch can support watching
//              for both input and output
//                events simultaneously
//   has_separate_rw_fd_watches
//              - boolean indicating whether it is possible to add separate
//              input and output watches for the
//                same fd. Either this or has_bidi_fd_watch must be true.
//   interrupt_after_fd_add
//              - boolean indicating if a loop interrupt must be forced after
//              adding/enabling an fd watch.
//   interrupt_after_signal_add
//              - boolean indicating if a loop interrupt must be forced after
//              adding or enabling a signal
//                watch.
//   supports_non_oneshot_fd
//              - boolean; if true, event_dispatch can arm an fd watch without
//              ONESHOT and returning zero
//                events from receive_fd_event (the event notification function)
//                will leave the descriptor armed. If false, all fd watches are
//                effectively ONESHOT (they can be re-armed immediately after
//                delivery by returning an appropriate event flag mask).
//   full_timer_support
//              - boolean indicating that the monotonic and system clocks are
//              actually different clocks and
//                that timers against the system clock will work correctly if
//                the system clock time is adjusted. If false, the monotic clock
//                may not be present at all (monotonic clock will map to system
//                clock), and timers against either clock are not guaranteed to
//                work correctly if the system clock is adjusted.

#if DASYNQ_HAVE_EPOLL <= 0
#	if _POSIX_TIMERS > 0
#		include "dasynq/posixtimer.hpp"
namespace dasynq {
template <typename T, bool provide_mono_timer = true>
using timer_events = posix_timer_events<T, provide_mono_timer>;
} // namespace dasynq
#	else
#		include "dasynq/itimer.hpp"
namespace dasynq {
template <typename T, bool provide_mono_timer = true>
using timer_events = itimer_events<T, provide_mono_timer>;
} // namespace dasynq
#	endif
#endif
...
#		include "dasynq/childproc.hpp"
#		include "dasynq/kqueue_macos.hpp"
namespace dasynq {
template <typename T>
using loop_t = macos_kqueue_loop<
    timer_events<child_proc_events<interrupt_channel<T>>, false>>;
using loop_traits_t = macos_kqueue_traits;
} // namespace dasynq
#	else
#		include "dasynq/childproc.hpp"
#		include "dasynq/kqueue.hpp"
namespace dasynq {
template <typename T>
using loop_t =
    kqueue_loop<timer_events<child_proc_events<interrupt_channel<T>>, false>>;
using loop_traits_t = kqueue_traits;
} // namespace dasynq
#	endif
#elif DASYNQ_HAVE_EPOLL
#	include "dasynq/childproc.hpp"
#	include "dasynq/epoll.hpp"
#	include "dasynq/timerfd.hpp"
namespace dasynq {
template <typename T>
using loop_t =
    epoll_loop<interrupt_channel<timer_fd_events<child_proc_events<T>>>>;
...
#		include "dasynq/pselect.hpp"
namespace dasynq {
template <typename T>
using loop_t = pselect_events<
    timer_events<interrupt_channel<child_proc_events<T>>, false>>;
using loop_traits_t = select_traits;
} // namespace dasynq
#	else
#		include "dasynq/select.hpp"
namespace dasynq {
template <typename T>
using loop_t =
    select_events<timer_events<interrupt_channel<child_proc_events<T>>, false>>;
using loop_traits_t = select_traits;
} // namespace dasynq
#	endif
#endif
...
#include "dasynq/mutex.hpp"

#include "dasynq/basewatchers.hpp"
...
enum class rearm {
	...
	REARM,
	/** Disarm the event watcher so that it receives no further events, until it
	   is re-armed explicitly */
	DISARM,
...
public:
	void signal() { condvar.notify_one(); }
...
public:
	...
	static waitqueue_node<null_mutex> *unqueue() { return nullptr; }

	static waitqueue_node<null_mutex> *get_head() { return nullptr; }

	static waitqueue_node<null_mutex> *get_second() { return nullptr; }

	static bool check_head(waitqueue_node<null_mutex> & /*node*/) {
		return true;
	}
...
public:
	...
	waitqueue_node<T_Mutex> *get_head() { return head; }

	waitqueue_node<T_Mutex> *get_second() { return head->next; }

	bool check_head(waitqueue_node<T_Mutex> &node) { return head == &node; }
...
public:
	...
	template <typename Loop>
	static rearm process_fd_rearm(Loop &loop,
	                              typename Loop::base_fd_watcher *bfw,
	...
	template <typename Loop>
	static rearm
	process_primary_rearm(Loop &loop, typename Loop::base_bidi_fd_watcher *bdfw,
	                      rearm rearm_type) noexcept {
	...
	template <typename Loop>
	static rearm
	process_secondary_rearm(Loop &loop,
	                        typename Loop::base_bidi_fd_watcher *bdfw,
	                        base_watcher *outw, rearm rearm_type) noexcept {
		return loop.process_secondary_rearm(bdfw, outw, rearm_type);
	}

	template <typename Loop>
	static void process_signal_rearm(Loop &loop,
	                                 typename Loop::base_signal_watcher *bsw,
	                                 rearm rearm_type) noexcept {
	...
	template <typename Loop>
	static void
	process_child_watch_rearm(Loop &loop,
	                          typename Loop::base_child_watcher *bcw,
	                          rearm rearm_type) noexcept {
		loop.process_child_watch_rearm(bcw, rearm_type);
	}

	template <typename Loop>
	static void process_timer_rearm(Loop &loop,
	                                typename Loop::base_timer_watcher *btw,
	                                rearm rearm_type) noexcept {
...
// Do standard post-dispatch processing for a watcher. This handles the case of
// removing or re-queueing watchers depending on the rearm type. This is called
// from the individual watcher dispatch functions to handle REMOVE or REQUEUE
// re-arm values.
template <typename Loop>
void post_dispatch(Loop &loop, base_watcher *watcher, rearm rearm_type) {
...
template <typename Loop>
void post_dispatch(Loop &loop, bidi_fd_watcher<Loop> *bdfd_watcher,
                   base_watcher *out_watcher, rearm rearm_type) {
	auto *watcher = (base_watcher *)bdfd_watcher;
	if (rearm_type == rearm::REMOVE) {
		loop_access::get_base_lock(loop).unlock();
		loop_access::release_watcher(loop, watcher);
...
// The event_dispatch class serves as the base class (mixin) for the backend
// mechanism. It mostly manages queing and dequeing of events and maintains/owns
// the relevant data structures, including a mutex lock.
//
// The backend mechanism should call one of the receiveXXX functions to notify
// of an event received. The watcher will then be queued.
//
// In general the functions should be called with lock held. In practice this
// means that the event loop backend implementations (that deposit received
// events here) must obtain the lock; they are also free to use it to protect
...
private:
	...
	using base_signal_watcher =
	    dprivate::base_signal_watcher<typename traits_t::sigdata_t>;
	using base_child_watcher = dprivate::base_child_watcher;
	using base_timer_watcher = dprivate::base_timer_watcher;

	// Add a watcher into the queueing system (but don't queue it). Call with
	// lock held.
...
protected:
	...
	// Receive a signal; return true to disable signal watch or false to leave
	// enabled. Called with lock held.
	template <typename T>
	bool receive_signal(T & /*loop_mech*/, typename Traits::sigdata_t &siginfo,
	                    void *userdata) noexcept {
		auto *bwatcher = static_cast<base_signal_watcher *>(userdata);
		bwatcher->siginfo = siginfo;
		queue_watcher(bwatcher);
		return true;
	}

	// Receive fd event delivered from backend mechansim. Returns the desired
	// watch mask, as per set_fd_enabled, which can be used to leave the watch
	// disabled, re-enable it or re-enable one direction of a bi-directional
	// watcher.
	template <typename T>
	std::tuple<int, typename Traits::fd_s>
	receive_fd_event(T & /*loop_mech*/, typename Traits::fd_r /*fd_r*/,
	                 void *userdata, int flags) noexcept {
		auto *bfdw = static_cast<base_fd_watcher *>(userdata);
		...
		if (is_multi_watch) {
			auto *bbdw = dynamic_cast<base_bidi_fd_watcher *>(bwatcher);
		...
		if (is_multi_watch && !traits_t::has_separate_rw_fd_watches) {
			// If this is a bidirectional fd-watch, it has been disabled in
			// *both* directions as the event was delivered. However, the other
			// direction should not be disabled yet, so we need to re-enable:
			int in_out_mask = IN_EVENTS | OUT_EVENTS;
			if ((bfdw->watch_flags & in_out_mask) != 0) {
				// We need to re-enable the other channel now:
				return std::make_tuple(
				    (bfdw->watch_flags & in_out_mask) | ONE_SHOT, watch_fd_s);
				// We are the polling thread: don't need to interrupt polling,
				// even if it would normally be required.
			}
		}
	...
	// Child process terminated. Called with both the main lock and the reaper
	// lock held.
	void receive_child_stat(pid_t /*child*/, int status,
	                        void *userdata) noexcept {
		auto *watcher = static_cast<base_child_watcher *>(userdata);
		watcher->child_status = status;
		watcher->child_termd = true;
		queue_watcher(watcher);
	}

	void receive_timer_expiry(timer_handle_t & /*timer_handle*/, void *userdata,
	                          int intervals) noexcept {
		auto *watcher = static_cast<base_timer_watcher *>(userdata);
		watcher->intervals += intervals;
		queue_watcher(watcher);
	}

	// Pull a single event from the queue; returns nullptr if the queue is
	// empty. Call with lock held.
	...
	void issue_delete(base_watcher *watcher) noexcept {
		// This is only called when the attention lock is held, so if the
		// watcher is not active/queued now, it cannot become active (and will
		// not be reported with an event) during execution of this function.

		lock.lock();

		if (watcher->active) {
			// If the watcher is active, set deleteme true; the watcher will be
			// removed at the end of current processing (i.e. when active is set
			// false).
...
// This is the main event_loop implementation. It serves as an interface to the
// event loop backend (of which it maintains an internal instance). It also
// serialises polling the backend and provides safe deletion of watchers (see
// comments inline).
//
// The T_Mutex type parameter specifies the mutex type. A null_mutex can be used
// for a single-threaded event loop; std::mutex, or any mutex providing a
// compatible interface, can be used for a thread-safe event loop.
//
// The Traits type parameter specifies any required traits for the event loop.
// This specifies the back-end to use (backend_t, a template) and the basic
// back-end traits (backend_traits_t). The default is `default_traits<T_Mutex>'.
//
template <typename T_Mutex, typename Traits> class event_loop {
	...
	template <typename T>
	using event_dispatch = dprivate::event_dispatch<T, Traits>;
...
private:
	...
	using base_signal_watcher =
	    dprivate::base_signal_watcher<typename loop_traits_t::sigdata_t>;
	...
	// There is a complex problem with most asynchronous event notification
	// mechanisms when used in a multi-threaded environment. Generally, a file
	// descriptor or other event type that we are watching will be associated
	// with some data used to manage that event source. For example a web server
	// needs to maintain information about each client connection, such as the
	// state of the connection (what protocol version has been negotiated, etc;
	// if a transfer is taking place, what file is being transferred etc).
	//
	// However, sometimes we want to remove an event source (eg webserver wants
	// to drop a connection) and delete the associated data. The problem here is
	// that it is difficult to be sure when it is ok to actually remove the
	// data, since when requesting to unwatch the source in one thread it is
	// still possible that an event from that source is just being reported to
	// another thread (in which case the data will be needed).
	//
	// To solve that, we:
	// - allow only one thread to poll for events at a time, using a lock
	// - use the same lock to prevent polling, if we want to unwatch an event
	// source
	// - generate an event to interrupt any polling that may already be
	// occurring in
	//   another thread
	// - mark handlers as active if they are currently executing, and
	// - when removing an active handler, simply set a flag which causes it to
	// be
	//   removed once the current processing is finished, rather than removing
	//   it immediately.
	//
	// In particular the lock mechanism for preventing multiple threads polling
	// and for allowing polling to be interrupted is tricky. We can't use a
	// simple mutex since there is significant chance that it will be highly
	// contended and there are no guarantees that its acquisition will be fair.
	// In particular, we don't want a thread that is trying to unwatch a source
	// being starved while another thread polls the event source.
	//
	// So, we use two wait queues protected by a single mutex. The
	// "attn_waitqueue" (attention queue) is the high-priority queue, used for
	// threads wanting to unwatch event sources. The "wait_waitquueue" is the
	// queue used by threads that wish to actually poll for events, while they
	// are waiting for the main queue to become quiet.
	...
	// - The mutex only protects manipulation of the wait queues, and so should
	// not
	//   be highly contended.
	//
	// To claim the lock for a poll-wait, the procedure is:
	//    - check if the attn_waitqueue is empty;
	//    - if it is, insert node at the head, thus claiming the lock, and
	//    return
	...
	bool long_poll_running = false; // whether any thread is polling the backend
	                                // (with non-zero timeout)
	waitqueue<mutex_t> attn_waitqueue;
	waitqueue<mutex_t> wait_waitqueue;

	mutex_t &get_base_lock() noexcept { return loop_mech.lock; }
	...
	void register_fd(base_fd_watcher *callback, int fd, int eventmask,
	                 bool enabled, bool emulate = false) {
		std::lock_guard<mutex_t> guard(loop_mech.lock);

		loop_mech.prepare_watcher(callback);

		try {
			if (!loop_mech.add_fd_watch(fd, callback, eventmask | ONE_SHOT,
			                            enabled, emulate)) {
				callback->emulatefd = true;
				callback->emulate_enabled = enabled;
				if (enabled) {
					callback->event_flags = eventmask & IO_EVENTS;
					if ((eventmask & IO_EVENTS) != 0) {
	...
	// Register a bidi fd watcher. The watch_flags should already be set to the
	// eventmask to watch (i.e. eventmask == callback->watch_flags is a
	// pre-condition).
	void register_fd(base_bidi_fd_watcher *callback, int fd, int eventmask,
	                 bool emulate = false) {
		std::lock_guard<mutex_t> guard(loop_mech.lock);
		...
		try {
			...
			try {
				...
				if (backend_traits_t::has_separate_rw_fd_watches) {
					int r = loop_mech.add_bidi_fd_watch(
					    fd, callback, eventmask | ONE_SHOT, emulate);
					if ((r & IN_EVENTS) != 0) {
						callback->emulatefd = true;
						if ((eventmask & IN_EVENTS) != 0) {
							callback->watch_flags &= ~IN_EVENTS;
							requeue_watcher(callback);
						}
					} else if ((eventmask & IN_EVENTS) &&
					           backend_traits_t::interrupt_after_fd_add) {
						do_interrupt = true;
					}

					if ((r & OUT_EVENTS) != 0) {
						callback->out_watcher.emulatefd = true;
						if ((eventmask & OUT_EVENTS) != 0) {
							callback->watch_flags &= ~OUT_EVENTS;
							requeue_watcher(&callback->out_watcher);
						}
					} else if ((eventmask & OUT_EVENTS) &&
					           backend_traits_t::interrupt_after_fd_add) {
						do_interrupt = true;
					}
				} else {
					if (!loop_mech.add_fd_watch(fd, callback,
					                            eventmask | ONE_SHOT, true,
					                            emulate)) {
						callback->emulatefd = true;
						callback->out_watcher.emulatefd = true;
						if ((eventmask & IN_EVENTS) != 0) {
	...
	void set_fd_enabled(base_watcher *watcher, int fd, int watch_flags,
	...
	void set_fd_enabled_nolock(base_watcher *watcher, int fd, int watch_flags,
	                           bool enabled) noexcept {
		if (enabled) {
			loop_mech.enable_fd_watch_nolock(fd, watcher,
			                                 watch_flags | ONE_SHOT);
			if (backend_traits_t::interrupt_after_fd_add) {
				interrupt_if_necessary();
			}
		} else {
	...
	void register_child(base_child_watcher *callback, pid_t child) {
		...
		loop_mech.prepare_watcher(callback);
		try {
			loop_mech.add_child_watch_nolock(callback->watch_handle, child,
			                                 callback);
		} catch (...) {
			loop_mech.release_watcher(callback);
			throw;
	...
	void register_reserved_child(base_child_watcher *callback,
	                             pid_t child) noexcept {
		loop_mech.add_reserved_child_watch(callback->watch_handle, child,
		                                   callback);
	}

	void register_reserved_child_nolock(base_child_watcher *callback,
	                                    pid_t child) noexcept {
		loop_mech.add_reserved_child_watch_nolock(callback->watch_handle, child,
		                                          callback);
	}
	...
	// Stop watching a child process, but retain watch reservation so that
	// another child can be watched without running into resource allocation
	// issues.
	...
	void set_timer(base_timer_watcher *callBack, const timespec &timeout,
	               clock_type clock) noexcept {
		struct timespec interval {
			0, 0
		};
		loop_mech.set_timer(callBack->timer_handle, timeout, interval, true,
		                    clock);
	}
	...
	void set_timer_rel(base_timer_watcher *callBack, const timespec &timeout,
		...
		loop_mech.set_timer_rel(callBack->timer_handle, timeout, interval, true,
		                        clock);
	}
	...
	void set_timer_enabled(base_timer_watcher *callback, clock_type clock,
	                       bool enabled) noexcept {
		loop_mech.enable_timer(callback->timer_handle, enabled, clock);
	}

	void set_timer_enabled_nolock(base_timer_watcher *callback,
	                              clock_type clock, bool enabled) noexcept {
	...
	// Interrupt the current poll-waiter, if necessary - that is, if the loop is
	// multi-thread safe, and if there is currently another thread polling the
	// backend event mechanism.
	void interrupt_if_necessary() {
		wait_lock.lock();
		bool attn_q_empty =
		    attn_waitqueue
		        .is_empty(); // (always false for single-threaded loops)
		wait_lock.unlock();
	...
	// Acquire the attention lock (when held, ensures that no thread is polling
	// the AEN mechanism). This can be used to safely remove watches, since it
	// is certain that notification callbacks won't be run while the attention
	// lock is held. Any in-progress poll will be interrupted so that the lock
	// should be acquired quickly.
	void get_attn_lock(waitqueue_node<T_Mutex> &qnode) noexcept {
		...
		if (!attn_waitqueue.check_head(qnode)) {
			if (long_poll_running) {
				// We want to interrupt any in-progress poll so that the attn
				// queue will progress but we don't want to do that
				// unnecessarily. If we are 2nd in the queue then the head must
				// be doing the poll; interrupt it. Otherwise, we assume the 2nd
	...
	// Acquire the attention lock, but without interrupting any poll that's in
	// progress (prefer to fail in that case).
	bool poll_attn_lock(waitqueue_node<T_Mutex> &qnode) noexcept {
		...
		// Nobody's doing a long poll, wait until we're at the head of the attn
	...
	// Acquire the poll-wait lock (to be held when polling the AEN mechanism;
	// lower priority than the attention lock). The poll-wait lock is used to
	// prevent more than a single thread from polling the event loop mechanism
	// at a time; if this is not done, it is basically impossible to safely
	// deregister watches.
	...
	void release_lock(waitqueue_node<T_Mutex> & /*qnode*/) noexcept {
		...
		if (nhead != nullptr) {
			...
			// Nobody is waiting in attn_waitqueue (the high-priority queue) so
			// check in wait_waitqueue (the low-priority queue)
	...
	void process_signal_rearm(base_signal_watcher *bsw,
	                          rearm rearm_type) noexcept {
	...
	// Process rearm return from an fd_watcher, including the primary watcher of
	// a bidi_fd_watcher. Depending on the rearm value, we re-arm, remove, or
	// disarm the watcher, etc.
	rearm process_fd_rearm(base_fd_watcher *bfw, rearm rearm_type) noexcept {
		...
		if (emulatedfd) {
			...
			set_fd_enabled_nolock(bfw, bfw->watch_fd,
			                      bfw->watch_flags & (IN_EVENTS | OUT_EVENTS),
			                      true);
		} else if (rearm_type == rearm::DISARM) {
	...
	rearm process_primary_rearm(base_bidi_fd_watcher *bdfw,
	                            rearm rearm_type) noexcept {
		...
		if (rearm_type == rearm::REMOVE) {
			...
			if (!bdfw->write_removed) {
				if ((bdfw->watch_flags & IN_EVENTS) != 0) {
					bdfw->watch_flags &= ~IN_EVENTS;
					if (!emulatedfd) {
			...
			if (!emulatedfd) {
				loop_mech.remove_fd_watch_nolock(bdfw->watch_fd,
				                                 0 /* not used */);
			}
			return rearm::REMOVE;
		}
		if (rearm_type == rearm::DISARM) {
			bdfw->watch_flags &= ~IN_EVENTS;

			if (!emulatedfd) {
				if (!backend_traits_t::has_separate_rw_fd_watches) {
					int watch_flags =
					    bdfw->watch_flags & (IN_EVENTS | OUT_EVENTS);
					set_fd_enabled_nolock(bdfw, bdfw->watch_fd, watch_flags,
					                      watch_flags != 0);
				} else {
					loop_mech.disable_fd_watch_nolock(bdfw->watch_fd,
					                                  IN_EVENTS);
				}
			}
		} else if (rearm_type == rearm::REARM) {
			if (!emulatedfd) {
				...
				if (!backend_traits_t::has_separate_rw_fd_watches) {
					...
					set_fd_enabled_nolock(
					    bdfw, bdfw->watch_fd,
					    watch_flags & (IN_EVENTS | OUT_EVENTS), true);
				} else {
					set_fd_enabled_nolock(bdfw, bdfw->watch_fd, IN_EVENTS,
					                      true);
				}
			} else {
				bdfw->watch_flags &= ~IN_EVENTS;
	...
	// Process re-arm for the secondary (output) watcher in a Bi-direction Fd
	// watcher.
	rearm process_secondary_rearm(base_bidi_fd_watcher *bdfw,
	                              base_watcher *outw,
	                              rearm rearm_type) noexcept {
		...
		if (emulatedfd) {
			if (rearm_type == rearm::REMOVE) {
				...
				if ((bdfw->watch_flags & OUT_EVENTS) != 0) {
					bdfw->watch_flags &= ~OUT_EVENTS;
					rearm_type = rearm::REQUEUE;
				}
			}
		...
		if (rearm_type == rearm::REMOVE) {
			...
			if (!backend_traits_t::has_separate_rw_fd_watches) {
				int watch_flags = bdfw->watch_flags;
				set_fd_enabled_nolock(bdfw, bdfw->watch_fd,
				                      watch_flags & (IN_EVENTS | OUT_EVENTS),
				                      true);
			} else {
			...
			if (!backend_traits_t::has_separate_rw_fd_watches) {
				...
				set_fd_enabled_nolock(bdfw, bdfw->watch_fd,
				                      OUT_EVENTS | ONE_SHOT, true);
			}
		}
		return rearm_type;
	}

	void process_child_watch_rearm(base_child_watcher *bcw,
	                               rearm rearm_type) noexcept {
	...
	void process_timer_rearm(base_timer_watcher *btw,
	                         rearm rearm_type) noexcept {
		// Called with lock held
		if (rearm_type == rearm::REARM) {
	...
	bool process_events(int limit) noexcept {
		...
		while (pqueue != nullptr) {
			...
			// (Above variables are initialised only to silence compiler
			// warnings).

			if (pqueue->watchType == watch_type_t::SECONDARYFD) {
				// construct a pointer to the main watcher, using integer
				// arithmetic to avoid undefined pointer arithmetic:
				auto rp = (uintptr_t)pqueue;

				// Here we take the offset of a member from a
				// non-standard-layout class, which is specified to have
				// undefined result by the C++ language standard, but which in
				// practice works fine:
				_Pragma("GCC diagnostic push")
				    _Pragma("GCC diagnostic ignored \"-Winvalid-offsetof\"")
				        rp -= offsetof(base_bidi_fd_watcher, out_watcher);
				_Pragma("GCC diagnostic pop") bbfw = (base_bidi_fd_watcher *)rp;
...
public:
	...
	template <typename D>
	using fd_watcher_impl = dprivate::fd_watcher_impl<my_event_loop_t, D>;
	template <typename D>
	using bidi_fd_watcher_impl =
	    dprivate::bidi_fd_watcher_impl<my_event_loop_t, D>;
	template <typename D>
	using signal_watcher_impl =
	    dprivate::signal_watcher_impl<my_event_loop_t, D>;
	template <typename D>
	using child_proc_watcher_impl =
	    dprivate::child_proc_watcher_impl<my_event_loop_t, D>;
	template <typename D>
	using timer_impl = dprivate::timer_impl<my_event_loop_t, D>;

	// Poll the event loop and process any pending events (up to a limit). If no
	// events are pending, wait for and process at least one event.
	void run(int limit = -1) noexcept {
		...
		while (!process_events(limit)) {
			// Pull events from the AEN mechanism and insert them in our
			// internal queue:
			get_pollwait_lock(qnode);
	...
	//   force_update (default = false) - if true, the time returned will be
	//   updated from
	//       the system rather than being a previously cached result. It may be
	//       more accurate, but note that reading from a system clock may be
	//       relatively expensive.
	void get_time(timespec &ts, clock_type clock,
	              bool force_update = false) noexcept {
		loop_mech.get_time(ts, clock, force_update);
	}

	void get_time(time_val &tv, clock_type clock,
	              bool force_update = false) noexcept {
...
using event_loop_n = event_loop<null_mutex>;
using event_loop_th = event_loop<std::mutex>;

namespace dprivate {

// Posix signal event watcher
template <typename EventLoop>
class signal_watcher : private dprivate::base_signal_watcher<
                           typename EventLoop::loop_traits_t::sigdata_t> {
...
public:
	...
	// call.
	inline void add_watch(event_loop_t &eloop, int signo,
	...
	template <typename T>
	static signal_watcher<event_loop_t> *add_watch(event_loop_t &eloop,
	                                               int signo, T watch_hndlr) {
		class lambda_sig_watcher
		    : public signal_watcher_impl<event_loop_t, lambda_sig_watcher> {
		private:
			T watch_hndlr;

		public:
			explicit lambda_sig_watcher(T watch_handlr_a)
			    : watch_hndlr(watch_handlr_a) {
				//
			}
			...
			void watch_removed() noexcept override { delete this; }
		};

		auto *lsw = new lambda_sig_watcher(watch_hndlr);
		lsw->add_watch(eloop, signo);
		return lsw;
	}

	// virtual rearm received(EventLoop &eloop, int signo, siginfo_p siginfo) =
	// 0;
};
...
class signal_watcher_impl : public signal_watcher<EventLoop> {
	void dispatch(void *loop_ptr) noexcept override {
		...
		auto rearm_type = static_cast<Derived *>(this)->received(
		    loop, this->siginfo.get_signo(), this->siginfo);
		...
		if (rearm_type != rearm::REMOVED) {
			...
			this->active = false;
			if (this->deleteme) {
				// We don't want a watch that is marked "deleteme" to re-arm
				// itself.
...
protected:
	// Set the types of event to watch. Only supported if
	// loop_traits_t_t::has_bidi_fd_watch is true; otherwise has unspecified
	// behavior. Only safe to call from within the callback handler (fdEvent).
	// Might not take effect until the current callback handler returns with
	// REARM.
	void set_watch_flags(int newFlags) { this->watch_flags = newFlags; }
...
public:
	...
	void add_watch(event_loop_t &eloop, int fd, int flags, bool enabled = true,
	               int prio = DEFAULT_PRIORITY) {
		base_watcher::init();
		this->priority = prio;
		this->watch_fd = fd;
	...
	void add_watch_noemu(event_loop_t &eloop, int fd, int flags,
	                     bool enabled = true, int prio = DEFAULT_PRIORITY) {
		base_watcher::init();
		this->priority = prio;
		this->watch_fd = fd;
		this->watch_flags = flags;
	...
	int get_watched_fd() { return this->watch_fd; }
	...
	void set_enabled(event_loop_t &eloop, bool enable) noexcept {
		...
		if (this->emulatefd) {
			...
			eloop.set_fd_enabled_nolock(this, this->watch_fd, this->watch_flags,
			                            enable);
		}
	...
	// Add an Fd watch via a lambda. The watch is allocated dynamically and
	// destroys itself when removed from the event loop.
	template <typename T>
	static fd_watcher<EventLoop> *add_watch(event_loop_t &eloop, int fd,
	                                        int flags, T watchHndlr) {
		class lambda_fd_watcher
		    : public fd_watcher_impl<event_loop_t, lambda_fd_watcher> {
		private:
			T watchHndlr;

		public:
			explicit lambda_fd_watcher(T watchHandlr_a)
			    : watchHndlr(watchHandlr_a) {
				//
			}
		...
		auto *lfd = new lambda_fd_watcher(watchHndlr);
...
class fd_watcher_impl : public fd_watcher<EventLoop> {
	void dispatch(void *loop_ptr) noexcept override {
		...
		// In case emulating, clear enabled here; REARM or explicit set_enabled
		// will re-enable.
		this->emulate_enabled = false;

		loop_access::get_base_lock(loop).unlock();

		auto rearm_type = static_cast<Derived *>(this)->fd_event(
		    loop, this->watch_fd, this->event_flags);
...
// A Bi-directional file descriptor watcher with independent read- and write-
// channels. This watcher type has two event notification methods which can both
// potentially be active at the same time.
template <typename EventLoop>
	...
	void set_watch_enabled(EventLoop &eloop, bool in, bool b) {
		...
		if (!watcher->emulatefd) {
			if (EventLoop::loop_traits_t::has_separate_rw_fd_watches) {
				...
				eloop.set_fd_enabled_nolock(
				    this, this->watch_fd,
				    (this->watch_flags & IO_EVENTS) | ONE_SHOT,
				    (this->watch_flags & IO_EVENTS) != 0);
			}
		} else {
			// emulation: if enabling a previously disabled watcher, must queue
			// now:
...
public:
	...
	// Set the watch flags, which enables/disables both the in-watch and the
	// out-watch accordingly.
	//
	// Concurrency: this method can only be called if
	//  - it does not enable a watcher that might currently be active
	///   - unless the event loop will not be polled while the watcher is
	///   active.
	// (i.e. it is ok to call setWatchFlags from within the readReady/writeReady
	// handlers if no other
	//  thread will poll the event loop; it is always ok to *dis*able a watcher
	//  that might be active, though the re-arm action returned by the callback
	//  may undo the effect).
	void set_watches(event_loop_t &eloop, int new_flags) noexcept {
		std::lock_guard<mutex_t> guard(eloop.get_base_lock());
		bool use_emulation = this->emulatefd || this->out_watcher.emulatefd;
		if (use_emulation ||
		    EventLoop::loop_traits_t::has_separate_rw_fd_watches) {
			...
			eloop.set_fd_enabled_nolock((dprivate::base_watcher *)this,
			                            this->watch_fd,
			                            this->watch_flags & IO_EVENTS, true);
		}
	}
	...
	//
	// Can fail with std::bad_alloc or std::system_error.
	void add_watch(event_loop_t &eloop, int fd, int flags,
	               int inprio = DEFAULT_PRIORITY,
	               int outprio = DEFAULT_PRIORITY) {
		base_watcher::init();
	...
	template <typename T>
	static bidi_fd_watcher<event_loop_t> *add_watch(event_loop_t &eloop, int fd,
	                                                int flags, T watch_hndlr) {
		class lambda_bidi_watcher
		    : public bidi_fd_watcher_impl<event_loop_t, lambda_bidi_watcher> {
		private:
			T watch_hndlr;

		public:
			explicit lambda_bidi_watcher(T watch_handlr_a)
			    : watch_hndlr(watch_handlr_a) {
				//
			}
		...
		auto *lfd = new lambda_bidi_watcher(watch_hndlr);
		lfd->add_watch(eloop, fd, flags);
		return lfd;
	}
...
class bidi_fd_watcher_impl : public bidi_fd_watcher<EventLoop> {
	void dispatch(void *loop_ptr) noexcept override {
		...
		auto rearm_type =
		    static_cast<Derived *>(this)->read_ready(loop, this->watch_fd);
		...
		if (rearm_type != rearm::REMOVED) {
			...
			rearm_type =
			    loop_access::process_primary_rearm(loop, this, rearm_type);
	...
	void dispatch_second(void *loop_ptr) noexcept override {
		...
		auto rearm_type =
		    static_cast<Derived *>(this)->write_ready(loop, this->watch_fd);
		...
		if (rearm_type != rearm::REMOVED) {
			...
			rearm_type = loop_access::process_secondary_rearm(
			    loop, this, &outwatcher, rearm_type);
...
public:
	...
	// send a signal to this process, if it is still running, in a race-free
	// manner. return is as for POSIX kill(); return is -1 with errno=ESRCH if
	...
	// reservation (it will always fail) - check
	// loop_traits_t::supports_childwatch_reservation.
	void reserve_watch(event_loop_t &eloop) { eloop.reserve_child_watch(this); }

	void unreserve(event_loop_t &eloop) { eloop.unreserve(this); }

	// Register a watcher for the given child process with an event loop.
	// Registration can fail with std::bad_alloc.
	// Note that in multi-threaded programs, use of this function may be prone
	// to a race condition such that the child terminates before the watcher is
	// registered.
	void add_watch(event_loop_t &eloop, pid_t child,
	               int prio = DEFAULT_PRIORITY) {
		base_watcher::init();
	...
	// Note that in multi-threaded programs, use of this function may be prone
	// to a race condition such that the child terminates before the watcher is
	// registered; use the "fork" member function to avoid this.
	void add_reserved(event_loop_t &eloop, pid_t child,
	                  int prio = DEFAULT_PRIORITY) noexcept {
		base_watcher::init();
		this->watch_pid = child;
		this->priority = prio;
	...
	void stop_watch(event_loop_t &eloop) noexcept { eloop.stop_watch(this); }

	// Fork and watch the child with this watcher on the given event loop.
	// If resource limitations prevent the child process from being watched, it
	// is terminated immediately (or if the implementation allows, never
	// started), and a suitable std::system_error or std::bad_alloc exception is
	// thrown. Returns:
	// - the child pid in the parent
	// - 0 in the child
	pid_t fork(event_loop_t &eloop, bool from_reserved = false,
	           int prio = DEFAULT_PRIORITY) {
		base_watcher::init();
		this->priority = prio;
	...
	// virtual rearm child_status(EventLoop &eloop, pid_t child, int status) =
	// 0;
};
...
class child_proc_watcher_impl : public child_proc_watcher<EventLoop> {
	void dispatch(void *loop_ptr) noexcept override {
		...
		auto rearm_type = static_cast<Derived *>(this)->status_change(
		    loop, this->watch_pid, this->child_status);
...
template <typename EventLoop> class timer : private base_timer_watcher {
...
public:
	...
	void add_timer(event_loop_t &eloop,
	               clock_type clock = clock_type::MONOTONIC,
	               int prio = DEFAULT_PRIORITY) {
		base_watcher::init();
		this->priority = prio;
		this->clock = clock;
	...
	void arm_timer(event_loop_t &eloop, const timespec &timeout,
	...
	void set_enabled(event_loop_t &eloop, clock_type clock,
	                 bool enabled) noexcept {
		std::lock_guard<mutex_t> guard(eloop.get_base_lock());
	...
	template <typename T>
	static timer<EventLoop> *add_timer(EventLoop &eloop, clock_type clock,
	                                   bool relative, const timespec &timeout,
		...
		public:
			explicit lambda_timer(T watch_handlr_a)
			    : watch_hndlr(watch_handlr_a) {
				//
			}
		...
		auto *lt = new lambda_timer(watch_hndlr);
...
class timer_impl : public timer<EventLoop> {
	void dispatch(void *loop_ptr) noexcept override {
		...
		this->intervals = 0;
		auto rearm_type =
		    static_cast<Derived *>(this)->timer_expiry(loop, intervals_report);
