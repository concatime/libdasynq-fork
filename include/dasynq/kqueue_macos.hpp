#ifndef DASYNQ_KQUEUE_MACOS_HPP_
#define DASYNQ_KQUEUE_MACOS_HPP_
...
template <class Base>
class macos_kqueue_loop : public signal_events<Base, true> {
	int kqfd; // kqueue fd
	...
	void process_events(struct kevent *events, int r) {
		...
		// Now we disable all received events, to simulate EV_DISPATCH. Note
		// that EV_DISPATH is actually available on MacOS, but we can't use it
		// due to the signal processing bug.
...
public:
	...
	void pull_events(bool do_wait) {
		...
		// using sigjmp/longjmp is ugly, but there is no other way. If a signal
		// that we're watching is received during polling, it will longjmp back
		// to here:
...
#endif /* DASYNQ_KQUEUE_MACOS_HPP_ */
