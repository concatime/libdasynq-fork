#ifndef DASYNQ_TIMERFD_HPP_
#define DASYNQ_TIMERFD_HPP_
...
// descriptors from regular file descriptors when events are reported by the
// loop mechanism so that we can correctly report a timer event or fd event.

// With a file descriptor or signal, we can use the item itself as the
// identifier for adding/removing watches. For timers, it's more complicated.
// When we add a timer, we are given a handle; we need to use this to modify the
// watch. We delegate the process of allocating a handle to a priority heap
...
private:
	...
	// the timerfd if there are no active timers).
	...
	void set_timer(timer_handle_t &timer_id, const time_val &timeouttv,
	               const time_val &intervaltv, timer_queue_t &queue, int fd,
...
public:
	...
	void set_timer(timer_handle_t &timer_id, const time_val &timeouttv,
		...
		switch (clock) {
		case clock_type::SYSTEM:
			set_timer(timer_id, timeout, interval, queue, systemtime_fd,
			          enable);
			break;
		case clock_type::MONOTONIC:
	...
	void set_timer_rel(timer_handle_t &timer_id, const time_val &timeout,
	                   const time_val &interval, bool enable,
	                   clock_type clock = clock_type::MONOTONIC) noexcept {
		time_val alarmtime;
