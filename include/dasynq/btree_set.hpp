#ifndef DASYNQ_BTREE_SET_HPP_
#define DASYNQ_BTREE_SET_HPP_
...
// A sorted set based on a B-Tree data structure, supporting pre-allocation of
// nodes.
...
private:
	struct septnode {
		...
		bool is_leaf() noexcept { return children[0] == nullptr; }
	...
	int next_sept =
	    1; // next num_allocd for which we need another septnode in reserve.

	// Note that sept nodes are always at least half full, except for the root
	// sept node. For up to N nodes, one sept node is needed;
	//        at N+1 nodes, three sept nodes are needed: a root and two leaves;
	//     for every N/2 nodes thereafter, an additional sept node may be
	//     required.
	// A simple approximation is, s = (n * 2 + N - 1) / N.
	// (Actually we get away with much less, if nodes have the same priority,
	// since they are then linked in list and effectively become a single node).

	void alloc_slot() {
		num_alloced++;

		if (__builtin_expect(static_cast<long>(num_alloced == next_sept), 0) !=
		    0) {
			if (++num_septs_needed > num_septs) {
				try {
					auto *new_res = new septnode();
	...
	void merge(septnode *lsibling, septnode *rsibling, int index) noexcept {
		...
		for (ri = 0; rsibling->hn_p[ri] != nullptr; ri++) {
			...
			if (lsibling->children[lchildren]) {
				lsibling->children[lchildren]->parent = lsibling;
			}
			lsibling->hn_p[lchildren]->parent = lsibling;
			lchildren++;
		}
	...
	start:
		...
		if (parent->children[0] == sept) {
			...
			if (rsibling->num_vals() + children + 1 <= N) {
				...
				if (sept->children[children + 1]) {
					sept->children[children + 1]->parent = sept;
				}
...
public:
	T &node_data(handle_t &hn) noexcept { return hn.nodedata.data; }
	...
	void deallocate(handle_t &hn) noexcept {
		...
		if (__builtin_expect(static_cast<long>(num_alloced < next_sept - N / 2),
		                     0) != 0) {
			next_sept -= N / 2;
			num_septs_needed--;
			if (num_septs_needed < num_septs - 1) {
				// Note the "-1" margin is to alleviate bouncing
	...
	bool insert(handle_t &hndl, P pval = P()) noexcept {
		...
		while (!srch_sept->is_leaf()) {
			...
			while (min <= max) {
				...
				if (srch_sept->hn_p[i] == nullptr ||
				    pval < srch_sept->prio[i]) {
					max = i - 1;
		...
		while (children == N) {
			...
			for (int i = N / 2; i < N; i++) {
				new_sibling->prio[i - N / 2] =
				    srch_sept->prio[i]; // new[0] = old[4]
				new_sibling->hn_p[i - N / 2] = srch_sept->hn_p[i];
				new_sibling->children[i - N / 2 + 1] =
				    srch_sept->children[i + 1];
				if (new_sibling->children[i - N / 2 + 1]) {
					new_sibling->children[i - N / 2 + 1]->parent = new_sibling;
				}
			...
			if (pval < srch_sept->prio[N / 2 - 1]) {
				...
				if (new_sibling->children[0]) {
					new_sibling->children[0]->parent = new_sibling;
				}
				...
				if (left_down) {
					left_down->parent = srch_sept;
				}
				if (right_down) {
					right_down->parent = new_sibling;
				}
			} else {
				auto o_prio = new_sibling->prio[0];
				auto o_hidx = new_sibling->hn_p[0];
				int i = 0;
				for (; i < (N / 2 - 1) && new_sibling->prio[i + 1] < pval;
				     i++) {
	...
	bool is_queued(handle_t &hndl) noexcept { return hndl.parent != nullptr; }

	bool empty() noexcept { return root_sept == nullptr; }
