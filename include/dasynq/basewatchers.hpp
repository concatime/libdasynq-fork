#ifndef DASYNQ_BASEWATCHERS_HPP_
#define DASYNQ_BASEWATCHERS_HPP_

// Dasynq: early declarations and base watchers.
//
// Here we define watcher functionality that is not dependent on the event loop
// type. In particular, base classes for the various watcher types. These are
// not part of the public API.
//
// In general access to the members of the basewatcher should be protected by a
// mutex. The event_dispatch lock is used for this purpose.
...
namespace dprivate {
// POSIX says that sigprocmask has unspecified behaviour if used in a
// multi-threaded process. We can use pthread_sigmask instead, but that may
// require linking with the threads library. This function is specialised to
// call one or the other depending on the mutex type:
...
template <>
inline void sigmaskf<null_mutex>(int how, const sigset_t *set, sigset_t *oset) {
	sigprocmask(how, set, oset);
}
} // namespace dprivate

// A template to generate suitable default loop traits for a given type of
// mutex:
...
using dary_heap_def = dary_heap<A, B, C>;
template <typename A, typename B>
using heap_def = stable_heap<dary_heap_def, A, B>;

namespace {
// use empty handles (not containing basewatcher *) if the handles returned from
// the queue are handle references, because we can derive a pointer to the
// containing basewatcher via the address of the handle in that case:
constexpr bool use_empty_node =
    std::is_same<typename heap_def<empty_node, int>::handle_t_r,
                 typename heap_def<empty_node, int>::handle_t &>::value;

using node_type =
    std::conditional<use_empty_node, empty_node, base_watcher *>::type;
} // namespace
...
enum class watch_type_t { SIGNAL, FD, CHILD, SECONDARYFD, TIMER };

template <typename Traits, typename LoopTraits> class event_dispatch;

// For FD watchers:
// Use this watch flag to indicate that in and out events should be reported
// separately, that is, watcher should not be disabled until all watched event
// types are queued.
constexpr static int multi_watch = 4;

// Represents a queued event notification. Various event watchers derive from
// this type.
class base_watcher {
public:
	...
	static void set_priority(base_watcher &p, int prio) { p.priority = prio; }
	...
	// The dispatch function is called to process a watcher's callback. It is
	// the "real" callback function; it usually delegates to a user-provided
	// callback.
	virtual void dispatch(void *loop_ptr) noexcept {};

	// Bi-directional file descriptor watches have a secondary dispatch function
	// for the secondary watcher (i.e. the output watcher):
...
inline base_watcher *get_watcher(prio_queue_emptynode &q,
                                 prio_queue_emptynode::handle_t &n) {
	uintptr_t bptr = (uintptr_t)&n;
	_Pragma("GCC diagnostic push")
	    _Pragma("GCC diagnostic ignored \"-Winvalid-offsetof\"") bptr -=
	    offsetof(base_watcher, heap_handle);
	_Pragma("GCC diagnostic pop") return (base_watcher *)bptr;
}

inline dprivate::base_watcher *get_watcher(prio_queue_bwnode &q,
                                           prio_queue_bwnode::handle_t &n) {
	return q.node_data(n);
}

// Allocate queue handle:
inline void allocate_handle(prio_queue_emptynode &q,
                            prio_queue_emptynode::handle_t &n,
                            base_watcher *bw) {
	q.allocate(n);
}

inline void allocate_handle(prio_queue_bwnode &q,
                            prio_queue_bwnode::handle_t &n, base_watcher *bw) {
	q.allocate(n, bw);
}

// Base signal event - not part of public API
template <typename T_Sigdata> class base_signal_watcher : public base_watcher {
...
protected:
	...
	// watch_flags: for a regular fd_watcher, this specifies the events that the
	// watcher
	//              is watching (or was watching if disabled). For a
	//              bidi_fd_watcher, specifies the events that the watcher is
	//              currently watching (i.e. specifies which halves of the Bidi
	//              watcher are enabled).
...
protected:
	...
	// The main instance is the "input" watcher only; we keep a secondary
	// watcher with a secondary set of flags for the "output" watcher. Note that
	// some of the flags in the secondary watcher aren't used; it exists mainly
	// so that it can be queued independently of the primary watcher.
...
#endif /* DASYNQ_BASEWATCHERS_HPP_ */
