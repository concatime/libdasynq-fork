#ifndef DASYNQ_TIMERBASE_HPP_
#define DASYNQ_TIMERBASE_HPP_
...
// time_val provides a wrapper around struct timespec, which overloads operators
// appropriately.
class time_val {
...
public:
	...
	time_val(const struct timespec &t) noexcept { time = t; }
	...
	timespec &get_timespec() noexcept { return time; }

	const timespec &get_timespec() const noexcept { return time; }

	operator timespec() const noexcept { return time; }
};
...
inline time_val operator+(const time_val &t1, const time_val &t2) noexcept {
	...
	static_assert(std::numeric_limits<decltype(ns)>::max() >= 2000000000,
	              "type too small");
	if (ns >= 1000000000) {
		ns -= 1000000000;
		s++;
...
inline bool operator<(const time_val &t1, const time_val &t2) noexcept {
	if (t1.seconds() < t2.seconds())
		return true;
	if (t1.seconds() == t2.seconds() && t1.nseconds() < t2.nseconds())
		return true;
	return false;
}
...
static inline int divide_timespec(const struct timespec &num,
                                  const struct timespec &den,
...
public:
	...
	timer_data(void *udata = nullptr) noexcept
	    : interval_time(0, 0), expiry_count(0), enabled(true), userdata(udata) {
		// constructor
	}
};
...
public:
	bool operator()(const struct timespec &a,
...
#else
	// If there is no monotonic clock, map both clock_type::MONOTONIC and
	// clock_type::SYSTEM to a single clock (based on gettimeofday).
protected:
	inline timer_queue_t &queue_for_clock(clock_type clock) {
		return timer_queue;
	}

	inline bool timer_queues_empty() { return timer_queue.empty(); }
#endif

	// For the specified timer queue, issue expirations for all timers set to
	// expire on or before the given time (curtime).
	void process_timer_queue(timer_queue_t &queue,
	                         const struct timespec &curtime) noexcept {
		if (queue.empty())
			return;

		// Peek timer queue; calculate difference between current time and
		// timeout
		...
		while (*timeout <= curtime_tv) {
			...
			if (interval.seconds() == 0 && interval.nseconds() == 0) {
				...
				if (data.enabled) {
					...
					data.expiry_count = 0;
					Base::receive_timer_expiry(thandle, data.userdata,
					                           expiry_count);
				}
				if (queue.empty()) {
					break;
				...
				// Now we have to divide the time overrun by the period to find
				// the interval overrun. This requires a division of a value not
	...
	// Process timers based on the current clock time. If any timers have
	// expired, set do_wait to false; otherwise, if any timers are pending, set
	// ts to the delay before the next timer expires and set wait_ts to &ts. (If
	// no timers are active, none of the output parameters are set).
	inline void process_timers(clock_type clock, bool &do_wait, timespec &ts,
	                           timespec *&wait_ts) {
		timespec now;
	...
	// Process timers based on the current clock time. If any timers have
	// expired, set do_wait to false; otherwise, if any timers are pending, set
	// tv to the delay before the next timer expires and set wait_tv to &tv. (If
	// no timers are active, none of the output parameters are set).
	inline void process_timers(clock_type clock, bool &do_wait, timeval &tv,
	                           timeval *&wait_tv) {
		timespec now;
		auto &timer_q = this->queue_for_clock(clock);
		this->get_time(now, clock, true);
	...
	// Process monotonic timers based on the current clock time. If any timers
	// have expired, set do_wait to false; otherwise, if any timers are pending,
	// set ts to the delay before the next timer expires and set wait_ts to &ts.
	// (If no timers are active, none of the output parameters are set).
	inline void process_monotonic_timers(bool &do_wait, timespec &ts,
	                                     timespec *&wait_ts) {
	...
	// set ts to the delay before the next timer expires and set wait_ts to &ts.
	// (If no timers are active, none of the output parameters are set).
	inline void process_monotonic_timers(bool &do_wait, timeval &tv,
	                                     timeval *&wait_tv) {
...
public:
	...
	void get_time(timespec &ts, clock_type clock, bool force_update) noexcept {
		clockid_t posix_clock_id =
		    (clock == clock_type::MONOTONIC) ? CLOCK_MONOTONIC : CLOCK_REALTIME;
	...
	void add_timer_nolock(timer_handle_t &h, void *userdata,
	                      clock_type clock = clock_type::MONOTONIC) {
		this->queue_for_clock(clock).allocate(h, userdata);
	}

	void remove_timer(timer_handle_t &timer_id,
	                  clock_type clock = clock_type::MONOTONIC) noexcept {
		std::lock_guard<decltype(Base::lock)> guard(Base::lock);
		remove_timer_nolock(timer_id, clock);
	}

	void
	remove_timer_nolock(timer_handle_t &timer_id,
	                    clock_type clock = clock_type::MONOTONIC) noexcept {
	...
	void enable_timer(timer_handle_t &timer_id, bool enable,
	                  clock_type clock = clock_type::MONOTONIC) noexcept {
		std::lock_guard<decltype(Base::lock)> guard(Base::lock);
	...
	void
	enable_timer_nolock(timer_handle_t &timer_id, bool enable,
	                    clock_type clock = clock_type::MONOTONIC) noexcept {
		auto &timer_queue = this->queue_for_clock(clock);
		...
		if (expiry_count != 0 && enable) {
			...
			Base::receive_timer_expiry(timer_id, node_data.userdata,
			                           expiry_count);
		} else {
