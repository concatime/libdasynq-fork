#ifndef DASYNQ_DARYHEAP_HPP_
#define DASYNQ_DARYHEAP_HPP_
...
/**
 * Priority queue implementation based on a heap with parameterised fan-out. All
 * nodes are stored in a vector, with the root at position 0, and each node has
 * N child nodes, at positions (p * N + 1) through (p * N + N) where p is the
 * (parent) node position.
 *
 * With N=2, this is a binary heap. Higher values of N may give better
 * performance due to better cache locality, but also increase fan-out which
 * will (if too high) also reduce performance.
 *
 * The destructor will not clean up (destruct) objects that have been added to
 * the queue. If the destructor of the element type (T) is non-trivial, all
 * handles should be de-allocated before destroying the queue.
 *
 * Implementation details:
 *
 * Adding a node returns a "handle", which maintains an index into the heap.
 * When the position of a node in the heap changes, its handle must be updated
 * (the advantage is that changing priority of or removing a node does not
 * require a linear search for the node).
 *
 * Node data is actually stored as part of the handle, not in the queue.
 *
 * To add a node to the queue, it is inserted at the end and then "bubbled down"
 * to its correct location according to priority. To removing a node, the node
 * is replaced with the last node in the vector and then that is "bubbled up" to
...
private:
	static_assert(std::is_nothrow_move_assignable<P>::value,
	              "P must be no-except move assignable");
	...
	public:
		...
		heap_node(handle_t *hnd_p, const P &prio_p) noexcept(
...
public:
	// Handle to an element on the heap in the node buffer; also contains the
	// data associated with the node. (Alternative implementation would be to
	// store the heap data in a separate container, and have the handle be an
	// index into that container).
	struct handle_t {
		union hd_u_t {
			// The data member is kept in a union so it doesn't get
			// constructed/destructed automatically, and we can construct it
			// lazily.
		public:
	...
	// Initialise a handle (if it does not have a suitable constructor). Need
	// not do anything but may store a sentinel value to mark the handle as
	// inactive. It should not be necessary to call this, really.
...
public:
	T &node_data(handle_t &index) noexcept { return index.hd_u.hd; }
	...
	template <typename... U> void allocate(handle_t &hnd, U &&...u) {
		...
		// largest object size is PTRDIFF_MAX, so we expect the largest vector
		// is that / sizeof node:
		constexpr hindex_t max_allowed =
		    (std::numeric_limits<ptrdiff_t>::max() - 1) / sizeof(heap_node);
	...
	handle_t &get_root() noexcept { return *hvec[0].hnd; }

	P &get_root_priority() noexcept { return hvec[0].prio; }

	void pull_root() noexcept { remove_h(0); }

	void remove(handle_t &hnd) noexcept { remove_h(hnd.heap_index); }

	bool empty() noexcept { return hvec.empty(); }

	bool is_queued(handle_t &hnd) noexcept {
		return hnd.heap_index != (hindex_t)-1;
	}

	// Set a node priority. Returns true iff the node becomes the root node (and
	// wasn't before).
