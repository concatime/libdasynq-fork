#ifndef DASYNQ_POSIXTIMER_HPP_
#define DASYNQ_POSIXTIMER_HPP_
...
private:
	...
	// Set the timeout to match the first timer in the queue (disable the timer
	...
	bool receive_signal(T &loop_mech, sigdata_t &siginfo, void *userdata) {
		...
		if (siginfo.get_signo() == SIGALRM) {
			...
			if (!real_timer_queue.empty()) {
				...
				this->process_timer_queue(real_timer_queue,
				                          curtime.get_timespec());
...
public:
	...
	void set_timer(timer_handle_t &timer_id, const timespec &timeout,
	               const timespec &interval, bool enable,
	...
	void set_timer_rel(timer_handle_t &timer_id, const timespec &timeout,
	                   const timespec &interval, bool enable,
	                   clock_type clock = clock_type::MONOTONIC) noexcept {
	...
	void stop_timer_nolock(timer_handle_t &timer_id,
		...
		if (timer_queue.is_queued(timer_id)) {
			...
			if (was_first &&
			    (clock != clock_type::MONOTONIC || provide_mono_timer)) {
				set_timer_from_queue(timer, timer_queue);
			}
		}
	}
