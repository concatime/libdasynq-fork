#ifndef DASYNQ_PSELECT_HPP_
#define DASYNQ_PSELECT_HPP_
...
public:
	...
	void pull_events(bool do_wait) noexcept {
		...
		// This is horrible, but hopefully will be optimised well. POSIX gives
		// no way to combine signal sets other than this.
		...
		int r = pselect(nfds, &read_set_c, &write_set_c, &err_set, wait_ts,
		                &sigmask);
		...
		if (r == -1 || r == 0) {
			...
			if (r == 0) {
				if (!do_wait) {
					// At least on Mac OS, pselect doesn't seem to give us a
					// pending signal if we have a zero timeout. Force detection
					// using sigmask:
					sigset_t origmask;
