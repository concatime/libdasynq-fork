#ifndef DASYNQ_KQUEUE_HPP_
#define DASYNQ_KQUEUE_HPP_
...
//
// kqueue is available on BSDs and Mac OS X, though there are subtle differences
// from OS to OS.
//
// kqueue supports watching file descriptors (input and output as separate
// watches only), signals, child processes, and timers. Unfortunately support
// for the latter two is imperfect; it is not possible to reserve process
// watches in advance; timers can only be active, count down immediately when
// created, and cannot be reset to another time. For timers especially the
// problems are significant: we can't allocate timers in advance, and we can't
// even feasibly manage our own timer queue via a single kqueue-backed timer.
// Therefore, an alternate timer mechanism must be used together with kqueue.
...
public:
	...
	public:
		...
		// Note: OpenBSD doesn't have this; most other systems do. Technically
		// it is part of the STREAMS interface.
	...
	// File descriptor optional storage. If the mechanism can return the file
	// descriptor, this class will be empty, otherwise it can hold a file
	// descriptor.
	class fd_s {
	public:
		fd_s(int) {}
	...
	// File descriptor reference (passed to event callback). If the mechanism
	// can return the file descriptor, this class holds the file descriptor.
	// Otherwise, the file descriptor must be stored in an fd_s instance.
	class fd_r {
		int fd;

	public:
		int getFd(fd_s ss) { return fd; }
		fd_r(int nfd) : fd(nfd) {}
	};
...
// If we have no sigtimedwait implementation, we have to retrieve signal data by
// establishing a signal handler.

// We need to declare and define a non-static data variable, "siginfo_p", in
// this header, without violating the "one definition rule". The only way to do
// that (before C++17) is via a template, even though we don't otherwise need a
// template here:
...
inline void unprep_signal(int signo) { signal(signo, SIG_DFL); }
...
template <class Base> class kqueue_loop : public Base {
	...
	// delivery mechanism without having any connection to it. Whereas regular
	// signals can be queued (especially "realtime" signals, via sigqueue()),
	// kqueue just maintains a counter of delivery attempts and clears this when
	// we read the event. What this means is that kqueue won't necessarily tell
	// us if signals are pending, in the case that:
	//  1) it already reported the attempted signal delivery and
	//  2) more than instance one of the same signal was pending at that time
	//  and 3) no more deliveries of the same signal have been attempted in the
	//  meantime.
	// Also, kqueue won't tell us about signals that were pending at the time
	// the signal filter was added. Finally, because pending signals can be
	// merged, the count of delivery attempts provided by kqueue does not
	// necessarily match the number of signals actually pending.
	//
	// Note that POSIX allows for multiple instances of a signal to be pending
	// even on systems that don't support queueing of signals.
	//
	// Ultimately, this means we need to check for pending signals independently
	// of what kqueue tells us.
	...
	// The flag to specify poll() semantics for regular file readiness: that is,
	// we want ready-for-read to be returned even at end of file:
#if defined(NOTE_FILE_POLL)
	// FreeBSD:
	constexpr static int POLL_SEMANTICS = NOTE_FILE_POLL;
#else
	// Note that macOS has an "EV_POLL" defined that looks like it should give
	// poll semantics when passed as a flag. However, it is filtered at the
	// syscall entry so we cannot use it in kqueue. (The kernel uses it
	// internally to implement poll()).
	...
	void process_events(struct kevent *events, int r) {
		...
		for (int i = 0; i < r; i++) {
			if (events[i].filter == EVFILT_SIGNAL) {
				...
				int flags =
				    events[i].filter == EVFILT_READ ? IN_EVENTS : OUT_EVENTS;
				auto r = Base::receive_fd_event(*this, fd_r(events[i].ident),
				                                events[i].udata, flags);
				if (std::get<0>(r) == 0) {
					// we use EV_CLEAR to clear the EOF status of fifos/pipes
	...
	// Pull a signal from pending, and report it, until it is no longer pending
	// or the watch should be disabled. Call with lock held. Returns:  true if
	// watcher should be enabled, false if disabled.
...
public:
	 ...
	 *
	 * Throws std::system_error or std::bad_alloc if the event loop cannot be
	 * initialised.
	 */
	kqueue_loop() {
		kqfd = kqueue();
	...
	~kqueue_loop() { close(kqfd); }

	void set_filter_enabled(short filterType, uintptr_t ident, void *udata,
	                        bool enable) {
		// Note, on OpenBSD enabling or disabling filter will not alter the
		// filter parameters (udata etc); on OS X however, it will. Therefore we
		// set udata here (to the same value as it was originally set) in order
		// to work correctly on both kernels.
		struct kevent kev;
		int fflags = (filterType == EVFILT_READ) ? POLL_SEMANTICS : 0;
		EV_SET(&kev, ident, filterType, enable ? EV_ENABLE : EV_DISABLE, fflags,
		       0, udata);
	...
	// soft_fail:  true if unsupported file descriptors should fail by returning
	// false instead
	//             of throwing an exception
	// returns: true on success; false if file descriptor type isn't supported
	// and emulate == true throws:  std::system_error or std::bad_alloc on
	// failure
	bool add_fd_watch(int fd, void *userdata, int flags, bool enabled = true,
	                  bool emulate = false) {
		...
		struct kevent kev;
		EV_SET(&kev, fd, filter, EV_ADD | (enabled ? 0 : EV_DISABLE), fflags, 0,
		       userdata);
		if (kevent(kqfd, &kev, 1, nullptr, 0, nullptr) == -1) {
			// Note that kqueue supports EVFILT_READ on regular file fd's, but
			// not EVFILT_WRITE.
	...
	int add_bidi_fd_watch(int fd, void *userdata, int flags,
	                      bool emulate = false) {
#ifdef EV_RECEIPT
		struct kevent kev[2];
		struct kevent kev_r[2];
		short rflags =
		    EV_ADD | ((flags & IN_EVENTS) ? 0 : EV_DISABLE) | EV_RECEIPT;
		short wflags =
		    EV_ADD | ((flags & OUT_EVENTS) ? 0 : EV_DISABLE) | EV_RECEIPT;
	...
	// flags specifies which watch to remove; ignored if the loop doesn't
	// support separate read/write watches.
	...
	void enable_fd_watch(int fd, void *userdata, int flags) {
		set_filter_enabled((flags & IN_EVENTS) ? EVFILT_READ : EVFILT_WRITE, fd,
		                   userdata, true);
	}
	...
	void add_signal_watch_nolock(int signo, void *userdata) {
		...
		// We need to register the filter with the kqueue early, to avoid a race
		// where we miss signals:
		struct kevent evt;
		...
		// The signal might be pending already but won't be reported by kqueue
		// in that case. We can queue it immediately (note that it might be
		// pending multiple times, so we need to re-check once signal processing
		// finishes if it is re-armed).
...
public:
	...
	void pull_events(bool do_wait) {
		...
		// wait_ts remains null for an infinite wait; it is later set to either
		// a 0 timeout if do_wait is false (or if we otherwise won't wait due to
		// events being detected early) or is set to an appropriate timeout for
		// the next timer's timeout.
