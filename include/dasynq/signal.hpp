#ifndef DASYNQ_SIGNAL_HPP_
#define DASYNQ_SIGNAL_HPP_
...
// Support for the standard POSIX signal mechanisms. This can be used by
// backends that don't otherwise support receiving signals. It is not
// particularly nice (it involves using longjmp out of a signal handler, which
// POSIX mildly frowns upon) but it's really the only viable way to process
// signals together with file descriptor / other events and obtain the full
// siginfo_t data passed to the signal handler.
...
// that is via a template, even though we don't otherwise need a template here:
...
inline sigjmp_buf &get_sigreceive_jmpbuf() { return sig_capture::rjmpbuf; }

inline void unprep_signal(int signo) { signal(signo, SIG_DFL); }

inline siginfo_t *get_siginfo() { return &sig_capture::siginfo_cap; }
...
//
// Active (watched and enabled) signals are maintained as a signal mask, which
// either has active signals in the mask or inactive signals in the mask,
// depending on the mask_enables parameter. (if mask_enables is true, active
// signals are in the mask). Which is more convenient depends exactly on how the
// mask will be used.
//
template <class Base, bool mask_enables = false>
class signal_events : public Base {
...
protected:
	...
	// Get the active signal mask - identifying the set of signals which have an
	// enabled watcher. if mask_enables is true, the returned set contains the
	// active signals; otherwise, it contains all inactive signals.
	const sigset_t &get_active_sigmask() { return active_sigmask; }
	...
	// process a received signal, and update sigmask - which should reflect the
	// inverse of the active signal mask.
