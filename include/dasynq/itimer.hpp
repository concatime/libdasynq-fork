#ifndef DASYNQ_ITIMER_HPP_
#define DASYNQ_ITIMER_HPP_
...
// Timer implementation based on the (basically obsolete) POSIX itimer
// interface.

// With this timer implementation, we only have one real clock (the monotonic
// clock) that we can run a timer against. However, if the system has both
// clocks, we still maintain two separate queues. If provide_mono_timer is false
// we actually provide no system timer and rely on the event mechanism which
// extends this class to measure time and run timeouts (via
// process_monotonic_timers functions).

template <class Base, bool provide_mono_timer = true>
class itimer_events : public timer_base<Base> {
private:
	// Set the alarm timeout to match the first timer in the queue (disable the
	// alarm if there are no active timers).
	void set_timer_from_queue() {
		...
		if (!mono_timer_queue.empty()) {
			...
			// If we have a separate monotonic clock, we get the interval for
			// the expiry of the next monotonic timer and use the lesser of the
			// system interval and monotonic interval:
			time_val mono_newtime = mono_timer_queue.get_root_priority();

			time_val curtimev_mono;
			timer_base<Base>::get_time(curtimev_mono, clock_type::MONOTONIC,
			                           true);
		...
		if (interval_set && newalarm.it_value.tv_sec == 0 &&
		    newalarm.it_value.tv_usec == 0) {
			// We passed the timeout: set alarm to expire immediately (we must
			// use {0,1} as {0,0} disables the timer).
			// TODO: it would be better if we just processed the appropriate
			// timers here,
			//       but that is complicated to get right especially if the
			//       event loop is multi-threaded.
...
protected:
...
#ifdef CLOCK_MONOTONIC
	// We need to override the monotonic timer processing functions from
	// timer_base to check both timer queues, since we aren't actually providing
	// a separate timer for the system clock:

	inline void process_monotonic_timers(bool &do_wait, timeval &tv,
	                                     timeval *&wait_tv) {
		// We need to process both timer queues and set tv/wait_tv according to
		// which has the timer that will expire soonest:
		timer_base<Base>::process_timers(clock_type::MONOTONIC, do_wait, tv,
		                                 wait_tv);

		if (!do_wait) {
			timer_base<Base>::process_timers(clock_type::SYSTEM, do_wait, tv,
			                                 wait_tv);
		} else {
			timeval mono_tv = tv;
			timer_base<Base>::process_timers(clock_type::SYSTEM, do_wait, tv,
	...
	inline void process_monotonic_timers(bool &do_wait, timespec &ts,
		...
		// We need to process both timer queues and set tv/wait_tv according to
		// which has the timer that will expire soonest:
		timer_base<Base>::process_timers(clock_type::MONOTONIC, do_wait, ts,
		                                 wait_ts);

		if (!do_wait) {
			timer_base<Base>::process_timers(clock_type::SYSTEM, do_wait, ts,
			                                 wait_ts);
		} else {
			timespec mono_ts = ts;
			timer_base<Base>::process_timers(clock_type::SYSTEM, do_wait, ts,
	...
	// Process monotonic timers based on the current clock time. However, we
	// shadow the function and provide an implementation which also processes
	// the system clock timer queue.
	inline void process_monotonic_timers() {
		auto &mono_timer_queue = this->queue_for_clock(clock_type::MONOTONIC);
		if (!mono_timer_queue.empty()) {
			struct timespec curtime_mono;
			timer_base<Base>::get_time(curtime_mono, clock_type::MONOTONIC,
			                           true);
			timer_base<Base>::process_timer_queue(mono_timer_queue,
			                                      curtime_mono);
		}
	...
	void process_timers() {
		...
		if (provide_mono_timer) {
			auto &mono_timer_queue =
			    this->queue_for_clock(clock_type::MONOTONIC);
			if (!mono_timer_queue.empty()) {
				struct timespec curtime_mono;
				timer_base<Base>::get_time(curtime_mono, clock_type::MONOTONIC,
				                           true);
...
public:
	...
	// starts (if not started) a timer to timeout at the given time. Resets the
	// expiry count to 0.
	//   enable: specifies whether to enable reporting of timeouts/intervals
	void set_timer(timer_handle_t &timer_id, const time_val &timeouttv,
	               const time_val &intervaltv, bool enable,
	...
	void set_timer_rel(timer_handle_t &timer_id, const time_val &timeouttv,
	                   const time_val &intervaltv, bool enable,
	                   clock_type clock = clock_type::MONOTONIC) noexcept {
		timespec timeout = timeouttv;
		timespec interval = intervaltv;
	...
	void stop_timer(timer_handle_t &timer_id,
	                clock_type clock = clock_type::MONOTONIC) noexcept {
		std::lock_guard<decltype(Base::lock)> guard(Base::lock);
		stop_timer_nolock(timer_id, clock);
	}

	void stop_timer_nolock(timer_handle_t &timer_id,
	                       clock_type clock = clock_type::MONOTONIC) noexcept {
		auto &timer_queue = this->queue_for_clock(clock);
		if (timer_queue.is_queued(timer_id)) {
