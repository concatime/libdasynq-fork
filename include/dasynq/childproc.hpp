#ifndef DASYNQ_CHILDPROC_HPP_
#define DASYNQ_CHILDPROC_HPP_
...
// Map of pid_t to void *, with possibility of reserving entries so that
// mappings can be later added with no danger of allocator exhaustion
// (bad_alloc).
class pid_map {
...
public:
	...
	void reserve(pid_handle_t &hndl) { b_map.allocate(hndl); }

	void unreserve(pid_handle_t &hndl) noexcept { b_map.deallocate(hndl); }
...
private:
	...
	reaper_mutex_t
	    reaper_lock; // used to prevent reaping while trying to signal a process
...
public:
	...
	void add_child_watch_nolock(pid_watch_handle_t &handle, pid_t child,
	                            void *val) {
		child_waiters.add(handle, child, val);
	}

	void add_reserved_child_watch(pid_watch_handle_t &handle, pid_t child,
	                              void *val) noexcept {
		std::lock_guard<decltype(Base::lock)> guard(Base::lock);
		child_waiters.add_from_reserve(handle, child, val);
	}

	void add_reserved_child_watch_nolock(pid_watch_handle_t &handle,
	                                     pid_t child, void *val) noexcept {
		child_waiters.add_from_reserve(handle, child, val);
	}
	...
	// Get the reaper lock, which can be used to ensure that a process is not
	// reaped while attempting to signal it.
	reaper_mutex_t &get_reaper_lock() noexcept { return reaper_lock; }
	...
	template <typename T> void init(T *loop_mech) {
		...
		// On some systems a SIGCHLD handler must be established, or SIGCHLD
