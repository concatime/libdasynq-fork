#ifndef DASYNQ_EPOLL_HPP_
#define DASYNQ_EPOLL_HPP_
...
public:
	...
	class fd_s {
		...
		// Epoll doesn't return the file descriptor (it can, but it can't return
		// both file descriptor and user data).
		int fd;
	...
	class fd_r {
	public:
		int get_fd(fd_s ss) { return ss.fd; }
	};
		...
		for (int i = 0; i < r; i++) {
			...
			if (ptr == &sigfd) {
				...
				(events[i].events & EPOLLERR) &&
				    (flags |= IN_EVENTS | OUT_EVENTS | ERR_EVENTS);
				auto r = Base::receive_fd_event(*this, fd_r(), ptr, flags);
				if (std::get<0>(r) != 0) {
					enable_fd_watch_nolock(fd_r().get_fd(std::get<1>(r)), ptr,
					                       std::get<0>(r));
				}
			}
		}
...
public:
	...
	// and soft_fail == true throws:  std::system_error or std::bad_alloc on
	// failure
	...
	void disable_fd_watch(int fd, int flags) noexcept {
		...
		// Epoll documentation says that hangup will still be reported, need to
		// check whether this is really the case. Suspect it is really only the
		// case if EPOLLIN is set.
		if (epoll_ctl(epfd, EPOLL_CTL_MOD, fd, &epevent) == -1) {
