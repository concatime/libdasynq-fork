#ifndef DASYNQ_SELECT_HPP_
#define DASYNQ_SELECT_HPP_
...
template <class Base> class select_events : public signal_events<Base, true> {
	...
	void process_events(fd_set *read_set_p, fd_set *write_set_p,
	                    fd_set *error_set_p) {
		...
		for (int i = 0; i <= max_fd; i++) {
			if (FD_ISSET(i, read_set_p) || FD_ISSET(i, error_set_p)) {
				if (FD_ISSET(i, &read_set) && rd_udata[i] != nullptr) {
					// report read
					auto r = Base::receive_fd_event(*this, fd_r(i), rd_udata[i],
					                                IN_EVENTS);
					if (std::get<0>(r) == 0) {
						FD_CLR(i, &read_set);
					}
		...
		for (int i = 0; i <= max_fd; i++) {
			if (FD_ISSET(i, write_set_p)) {
				if (FD_ISSET(i, &write_set) && wr_udata[i] != nullptr) {
					// report write
					auto r = Base::receive_fd_event(*this, fd_r(i), wr_udata[i],
					                                OUT_EVENTS);
					if (std::get<0>(r) == 0) {
						FD_CLR(i, &write_set);
					}
